...
- 事件类型：`pipeline_update`、`demand_update`、`build_update`、`stage_change`、`error`、`metrics_update`
- 流水线节点每次状态变化自动广播
...
│  └────────────────────────────────────────────────────────┘   │
│                                                              │
│  ┌─ 需求队列（状态彩色标签）┐  ┌─ 构建日志（自动滚动）──┐  │
│  │ ID    标题    状态  来源  │  │ 时间  需求ID 步骤 状态  │  │
│  └──────────────────────────┘  └─────────────────────────┘   │
│                                                              │
│  ┌─ App 列表（含下载/评分/收入列）───────────────────────┐  │
│  └────────────────────────────────────────────────────────┘   │
│                                                              │
│  ┌─ 活动日志（实时滚动，最多 200 行，等宽字体）──────────┐  │
│  │ [12:34:56] Pipeline run abc123 started                 │  │
│  │ [12:34:58] Crawling Reddit r/AppIdeas...               │  │
│  │ [12:35:12] Found 23 raw demands                        │  │
│  └────────────────────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────┘
```
...
| `GET /api/demands` | 需求列表（分页、筛选） |
| `GET /api/apps` | App 列表（分页、搜索） |
| `GET /api/builds` | 构建日志 |
| `GET /api/stats` | 统计数据 |
| `GET /api/pipeline/status/{thread_id}` | LangGraph 流水线状态 |
| `POST /api/pipeline/trigger` | 手动触发流水线 |
