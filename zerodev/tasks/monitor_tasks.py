...
async def _collect_pipeline_metrics() -> Dict[str, Any]:
    ...
    # Celery worker stats.
    try:
        from zerodev.celery_app import celery as celery_app
...
async def _store_metrics(metrics: Dict[str, Any]) -> None:
    ...
    try:
        ...
        key = f"zerodev:metrics:{metrics['timestamp']}"
        r.setex(key, 86400, json.dumps(metrics))  # TTL 24h
        r.set("zerodev:metrics:latest", json.dumps(metrics))
        logger.debug("Metrics stored: %s", key)
    except Exception as exc:
        logger.warning("Failed to store metrics in Redis: %s", exc)


@celery.task(name="zerodev.tasks.monitor_tasks.collect_metrics")
...
@celery.task(name="zerodev.tasks.monitor_tasks.health_check")
def health_check() -> dict:
    ...
    # Database check.
    try:
        from zerodev.database import get_sync_session
